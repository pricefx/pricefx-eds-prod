...
export async function createModal(contentNodes, isFragment) {
...
export async function openModal({ fragmentUrl, block }) {
  if (fragmentUrl) {
    const path = fragmentUrl.startsWith('http') ? new URL(fragmentUrl, window.location).pathname : fragmentUrl;

    const fragment = await loadFragment(path);
    const { showModal } = await createModal(fragment?.childNodes, true);
    showModal();
  } else {
    const { showModal } = await createModal(block);
    showModal();
  }
}
